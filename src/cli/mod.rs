...
pub(crate) enum Subcommand {
    ...
    ///
    /// Pass `all` as the grammar to try every registered grammar on each input
    /// and pick the one yielding the lowest entropy (a per-input "best fit").
    LodAnalyzeEntropy {
        program: PathBuf,
        grammar: String,
        corpus: PathBuf,
...
fn gather_inputs(dir: &Option<PathBuf>, inputs: &[String], sort: bool) -> Vec<(PathBuf, Vec<u8>)> {
    gather_inputs_paths(dir, inputs, sort)
        .into_iter()
        .filter_map(|path| std::fs::read(&path).ok().map(|input| (path, input)))
        .collect::<Vec<_>>()
}
...
pub(crate) fn main() {
    ...
    match opts.subcmd {
        ...
        Subcommand::ShowCmplog { program, input } => {
            ...
            println!("cmplog: ({} entries)", res.cmplog.len());
        ...
        Subcommand::SmallestCrash {
            ...
            let mut sess = JitFuzzingSession::builder(mod_spec.clone())
                ...
                .build();
            sess.initialize(&mut stats);
            let mut lod_engine = lod.as_deref().map(crate::make_engine);
        ...
        Subcommand::LodCorpusEdges {
            ...
            let inputs = gather_inputs(&Some(corpus), &[], true);

            let engine = crate::make_engine(&grammar);
            ...
            let mut total_input_count = inputs.len();
        ...
        Subcommand::LodAnalyzeEntropy {
            ...
            let func_symbols: Vec<String> = mod_spec
                ...
                .collect();
            let mut inputs = gather_inputs(&Some(corpus), &[], true);
            // In `all` mode we try every registered grammar on each input and
            // pick the lowest-entropy fit, so there is no single dummy skeleton.
            let all_mode = grammar == "all";
            let grammar_names: Vec<String> = if all_mode {
                lod_formats::grammar_vtables()
                    .iter()
            ...
            if !all_mode {
                let dummy_bytes = lod_formats::get_dummy(&grammar).unwrap().1.clone();
                inputs.push((PathBuf::from("$dummy"), dummy_bytes));
            }
            let engines: Vec<(String, Box<dyn lod::ErasedEngine>)> = grammar_names
                .iter()
                .map(|name| {
                    let mut engine = crate::make_engine(name);
                ...
                .collect();
            // Tally of which grammar won each input (only meaningful in `all` mode).
            let mut chosen_counts = std::collections::BTreeMap::<String, usize>::new();
            ...
            eprintln!("Running {} input(s) ...", inputs.len());
            for (path, bytes) in &inputs {
                let is_dummy = path.to_string_lossy() == "$dummy";
                let input_entropy: u32;
                let root_kind: String;
                if all_mode {
                    // Try every grammar and keep the lowest-entropy fit. Only the
                    // winner is parsed for its root variant (the hot loop just
                    // measures entropy per grammar).
                    let mut best: Option<(usize, u32)> = None;
                    for (i, (_name, engine)) in engines.iter().enumerate() {
                        let e = engine.get_entropy(bytes) as u32;
                    ...
                    let (bi, be) = best.expect("at least one grammar registered");
                    let (chosen_name, chosen_engine) = &engines[bi];
                    input_entropy = be;
                    let variant = chosen_engine
                        .parse_exact_levels(bytes)
                        .into_iter()
                        .find(|hit| hit.is_root)
                        .map(|hit| hit.variant_name.to_string())
                        .unwrap_or_else(|| "-".to_string());
                    root_kind = format!("{chosen_name}:{variant}");
                    *chosen_counts.entry(chosen_name.clone()).or_insert(0) += 1;
                } else {
                    let engine = &engines[0].1;
                    input_entropy = if is_dummy {
                        0
                    } else {
                        engine.get_entropy(bytes) as u32
                    };
                    root_kind = engine
                        .parse_exact_levels(bytes)
                        .into_iter()
            ...
            if all_mode && !chosen_counts.is_empty() {
                // Summarize which grammar was the lowest-entropy fit most often.
                let mut ranked: Vec<(String, usize)> = chosen_counts.into_iter().collect();
                ranked.sort_by(|a, b| b.1.cmp(&a.1).then_with(|| a.0.cmp(&b.0)));
                let total: usize = ranked.iter().map(|(_, c)| c).sum();
                println!();
                println!("Chosen grammar summary ({total} input(s)):");
                for (name, count) in &ranked {
                    let pct = 100.0 * *count as f64 / total as f64;
                    println!("  {count:>5} ({pct:>5.1}%)  {name}");
                }
                if let Some((name, count)) = ranked.first() {
                    println!("Most chosen: {name} ({count} input(s))");
                }
            }
        }
    }
