...
pub(crate) enum Experiment {
    ...
    /// 4x97: -0.24 % (tied); DDF1: -1.12 % mean / -1.26 % median, worst of the
    /// field, no BH cell — a full point below the fully-pinned `LodSingleLevel`,
    /// so "switch op off" ≠ "engine pinned". Confirmed neutral-to-negative;
    /// retained as the structural control only.
    LodNoLevelSwitching,
    /// Single-level ablation (`lock_dummy_level`). Pins the whole tree to the LOD
    /// level the dummy re-parses to — the highest byte-exact tier below the
    /// dummy's generated tier — for the engine's lifetime: `feed` lifts to that
    /// level and absorbs only there, and `mutate` drops tier switching, the
    /// fresh-root reseed, and grammar-variant hops. Stricter than
    /// `LodNoLevelSwitching` (which only kills the dedicated switch op).
    /// DDF1: net-neutral mean (-0.12 % / -0.68 % median) but owns the only sig
    /// *wins* of the campaign — `freetype2-ftfuzzer` Â12 0.726 q=0.049 +
    /// `vorbis-decode` Â12 0.737 q=0.048 (both large) — against a `lewton-ogg`
    /// loss. Byte-exact-tier-lock helps font/audio decoders while costing the
    /// structural majority: a per-grammar level-lock preset candidate, not a
    /// global knob. Confirming pass queued.
    LodSingleLevel,
    ...
    /// from the rotation. Archival anchor. DDF1: honest re-test after the
    /// `make_feedback` bug fix — still -0.98 % mean / -0.94 % median, 5-18, no BH
    /// cell. The bug was not masking a win; two negative reads close the
    /// flat-feedback question definitively.
    LodEntropyFeedback,

    /// Like [`Experiment::LodEntropyFeedback`], but the custom func/edge
    /// input-size feedback value packs the input's *structural LOD depth*
    /// alongside its entropy (`ErasedEngine::get_level_entropy` →
    /// `lod::pack_level_entropy`). The depth occupies the high bits and is
    /// inverted, so the min-reduced per-edge feedback favors the most-structured
    /// representative reaching each edge, with entropy as the intra-level
    /// tiebreak. Probes whether per-level partitioning of the size frontier
    /// helps where flat entropy (`LodEntropyFeedback`) did not.
    /// DDF1 (first campaign): +0.18 % mean / +0.07 % weighted — the only
    /// positive-mean variant, and ~1.2 pp above flat `LodEntropyFeedback`, so
    /// per-level partitioning *is* the better feedback shape. But vs the
    /// no-feedback base it washes (median -0.05 %, 0 sig wins, 1 sig loss
    /// `lewton-ogg` Â12 0.238 q=0.034). On probation: one confirming pass, then
    /// close the host-feedback axis. Level-awareness belongs on the *level
    /// selection* code path, not a host-feedback dim.
    LodLevelEntropyFeedback,

    /// Sweep arm to understand *why* [`Experiment::LodEntropyFeedback`]
    /// regressed. Writes the same flat entropy custom-size value, but instead of
    /// enabling both custom feedback dimensions unconditionally, **randomly picks
    /// one configuration per session** (func-only / edge-only / both) via
    /// `self.rng` in `make_feedback`. The custom value is held fixed, so the only
    /// swept variable is the custom feedback *type*; results accumulate across the
    /// fleet's sessions for a finer-grained read than the all-on `LodEntropyFeedback`.
    LodCustomFeedbackSweep,
    ...
    /// Generation probe: hand-tuned, *static* generation knobs informed by
    /// `lod corpus-shape` aggregates over real corpora — the `Fresh` defaults
    /// (list length `0..4`, 50% `Some`, small byte buffers) are far below
    /// observed shapes (e.g. ttf `TtfTyped.tables` p50≈8, heif/jxl child lists,
    /// large IDAT/codestream byte buffers). Lifts the global generation
    /// distributions (`GenConfig::list_len` / `byte_len`) and the structural
    /// list-growth / duplicate / splice-append weights so `generate`, fresh-root,
    /// and list-append synthesize corpus-scaled structure. Static (no learning) —
    /// the cheap A/B baseline for [`Experiment::LodGenAdaptive`].
    LodGenTuned,
    /// Generation probe: *per-type adaptive* generation. Sets
    /// `GenConfig::adaptive`, so the engine accumulates an empirical
    /// per-`TypeId` profile (list lengths, option occupancy, byte sizes) from
    /// every input it is `feed`-d and samples it during `generate` / fresh-root /
    /// list-append. Same idea as `corpus-shape`'s distributions, applied online;
    /// adapts to the live campaign corpus rather than fixed constants. Runs on
    /// the `LodBest` base.
    LodGenAdaptive,

    /// Eviction-policy probe: `corpus.eviction = Eviction::Fifo` (drop the
    /// oldest bucket entry) instead of the base `MidIndex`. `Eviction` has
    /// carried four policies (`MidIndex`/`Fifo`/`Random`/`Reservoir`) since
    /// `lod-core`'s corpus was written; the 2026-05-20 A/B (cap_per_type=256,
    /// pre-cmplog/pre-LowEntropy/pre-combo) found `Fifo` strictly worse, and
    /// z5y8 (2026-07-08, current combo base) confirmed it a second time:
    /// -1.02 % mean, worst of the field, with one isolated BH win
    /// (`libtiff-read_rgba` Â12 0.842 q=0.0051) that doesn't change the
    /// picture. Axis closed for good; kept as an archival control only.
    LodEvictFifo,
    /// Eviction-policy probe: `corpus.eviction = Eviction::Random` (overwrite
    /// a uniformly random bucket slot on overflow). Untested until z5y8
    /// (2026-07-08): -0.07 % mean / +0.70 % median, 15-8 better, no BH cell —
    /// washed near-neutral alongside `LodEvictFifo`/`LodEvictReservoir`.
    /// Archival control only.
    LodEvictRandom,
    /// Eviction-policy probe: `corpus.eviction = Eviction::Reservoir`
    /// (Vitter's R, accept-probability `cap/seen`). The 2026-05-20 A/B
    /// (same stale cap256 base as `LodEvictFifo`) washed at +0.29 % mean;
    /// z5y8 (2026-07-08, current combo base) confirmed the wash again
    /// (-0.13 % mean, 12-11 better, no BH cell) while posting the most
    /// rank-1 wins (8/23) of the grid — still not distinguishable from
    /// `MidIndex`. Eviction-policy axis closed for good; archival control
    /// only.
    LodEvictReservoir,
    /// Corpus-dedup probe: `corpus.unique = true` — a push whose
    /// `subtree_hash` matches an entry already in the bucket is dropped.
    /// Follow-up to the z5y8 eviction campaign (2026-07-08, washed 68/69 ns):
    /// `register_into`'s recursive walk pushes entries in intra-input
    /// correlated bursts, so buckets fill with duplicates and `pick` is
    /// implicitly frequency-weighted toward common values. Dedup attacks
    /// that directly — the bucket becomes a set of distinct donors — and
    /// removes the burst-flooding weakness that made recency policies
    /// fragile. Runs on the base (`MidIndex` eviction) otherwise untouched.
    /// VBkL (2026-07-09) first read: +0.57 % mean / +0.57 % median final,
    /// +0.59 %/+0.48 % weighted, 14-8 better (final) / 15-7 (weighted), mean
    /// Â12 0.528/0.526 — cleanest directional case since the combo
    /// promotion, but zero BH cells yet. Confirming pass queued.
    LodCorpusUnique,

    /// Mutation-kind ablation: `weights.custom = 0.0`, disabling
    /// `MutationKind::Custom` (the whole-node custom mutator a type opts into
    /// via `#[lod_node(mutate = ...)]`; base default `2.0`). Landed
    /// 2026-07-14 (`f18388e` "add custom mutations"); the only opted-in type
    /// so far is `sql::Word::canonicalize_word`, which re-syncs a token's
    /// `keyword` classification with its `value` after independent
    /// structural mutation desyncs them (sqlparser dispatches on
    /// `Word.keyword`, so a demoted `SELECT`/`FROM` breaks the
    /// `Tokens -> Ast` lift without it). Isolates whatever that mutator is
    /// worth on `sql` specifically, and doubles as a sanity check that the
    /// weight is a true no-op on every grammar without an opted-in type. See
    /// `lod-sketch/experiment-log.md` 2026-07-15.
    LodCustomOff,
    /// Mutation-kind probe: doubles the five list ops added 2026-07-15
    /// (`eb11257` "misc") — `list_slice` (drop a contiguous run),
    /// `list_insert` (insert `T::fresh` at a random index), `list_insert_dup`
    /// (clone-and-insert an existing element), `list_truncate` (shorten to a
    /// prefix), `list_range_splice` (insert a contiguous run from self/
    /// corpus) — `0.5/1.0/0.5/0.5/0.5 → 1.0/2.0/1.0/1.0/1.0`. Deliberately
    /// narrower than the retired [`Experiment::LodStructHeavy`] (which scaled
    /// the pre-existing `list_dup`/`list_shuffle` alongside reshape ops), so
    /// any effect is attributable to the newly-added ops rather than a rerun
    /// of the closed mutation-balance axis. See `lod-sketch/experiment-log.md`
    /// 2026-07-15.
    LodListExpanded,
}
...
impl Experiment {
    ...
    fn lod_config(self) -> lod::EngineConfig {
        ...
        match self {
            ...
            Experiment::LodSingleLevel => {
                // Pin to the dummy's LOD level. The engine reads the target depth
                // from its vtable and forces `mutation.lock_grammar_variants`, so
                // the bool drives the whole ablation; `lod_switch_inv = 0` is
                // redundant belt-and-suspenders.
                cfg.lock_dummy_level = true;
                cfg.lod_switch_inv = 0;
            }
            Experiment::LodBest
            | Experiment::LodCombo
            ...
            Experiment::LodLevelEntropyFeedback => {
                // LowEntropy comes from the base block. The custom-size value the
                // worker writes (depth-packed entropy) and the feedback dims live
                // in `worker.rs` / `make_feedback`.
            }
            Experiment::LodCustomFeedbackSweep => {
                // LowEntropy comes from the base block. The randomized custom
                // feedback dims live in `make_feedback`; the worker writes the same
                // flat entropy value as `LodEntropyFeedback`.
            }
            ...
            Experiment::LodGenTuned => {
                // Static generation knobs informed by `lod corpus-shape`: real
                // list lengths and byte buffers dwarf the `Fresh` `0..4` / small
                // defaults. Lift the global generation distributions and the
                // list-growth / dup / splice-append ops so synthesized structure
                // is corpus-scaled. (Hypothesis values, to be validated by A/B.)
                cfg.generation.list_len = Some((0, 12));
                cfg.generation.byte_len = Some((0, 256));
                let w = &mut cfg.mutation.weights;
                w.list_append *= 2.0;
                w.list_dup *= 3.0;
                w.splice_append *= 2.0;
            }
            Experiment::LodGenAdaptive => {
                // Per-type adaptive generation: the engine learns list-length /
                // option / byte distributions from fed inputs (keyed by element
                // TypeId) and samples them during generation. Runs on the base.
                cfg.generation.adaptive = true;
            }
            Experiment::LodEvictFifo => {
                cfg.corpus.eviction = lod::Eviction::Fifo;
            }
            Experiment::LodEvictRandom => {
                cfg.corpus.eviction = lod::Eviction::Random;
            }
            Experiment::LodEvictReservoir => {
                cfg.corpus.eviction = lod::Eviction::Reservoir;
            }
            Experiment::LodCorpusUnique => {
                cfg.corpus.unique = true;
            }
            Experiment::LodCustomOff => {
                cfg.mutation.weights.custom = 0.0;
            }
            Experiment::LodListExpanded => {
                let w = &mut cfg.mutation.weights;
                w.list_slice *= 2.0;
                w.list_insert *= 2.0;
                w.list_insert_dup *= 2.0;
                w.list_truncate *= 2.0;
                w.list_range_splice *= 2.0;
            }
...
impl Orchestrator {
    fn new(
        ...
        let mut lod_options = if let Some(engine) = &opts.g.lod {
            let engine = Box::leak(Box::new(engine.clone()));
            vec![engine.as_str()]
        } else if opts.experiment.is_some_and(|x| x.is_lod()) {
    ...
    pub fn suggest(&mut self) -> Config {
        ...
        opts.live_bbs = false;
        opts.func_input_size_custom = matches!(
            self.opts.experiment,
            Some(Experiment::LodEntropyFeedback | Experiment::LodLevelEntropyFeedback)
        );
        opts.edge_input_size_custom = matches!(
            self.opts.experiment,
            Some(Experiment::LodEntropyFeedback | Experiment::LodLevelEntropyFeedback)
        );

        // `LodCustomFeedbackSweep`: randomly enable one custom-feedback-type
        // configuration per session to isolate which dimension drives the
        // `LodEntropyFeedback` regression. The custom value is fixed (entropy,
        // written in worker.rs); only the feedback type is swept.
        if matches!(
            self.opts.experiment,
            Some(Experiment::LodCustomFeedbackSweep)
        ) {
            let (func, edge) = match self.rng.random_range(0..3) {
                0 => (true, false), // func custom only
                1 => (false, true), // edge custom only
                _ => (true, true),  // both (the original LodEntropyFeedback shape)
            };
            opts.func_input_size_custom = func;
            opts.edge_input_size_custom = edge;
        }
